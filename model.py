...
class ES(torch.nn.Module):
    ...
    def __init__(self, observation_space, action_space,
        ...
        if self.a3c_net:
            ...
            self.bnconv1 = nn.BatchNorm2d(16, momentum=1.)
            
            self.conv2 = nn.Conv2d(16, 32, 4, stride=2)
            self.bnconv2 = nn.BatchNorm2d(32, momentum=1.)
            ...
            self.bnconv1 = nn.BatchNorm2d(32, momentum=1.)
            
            self.conv2 = nn.Conv2d(32, 32, 3, stride=2, padding=1)
            self.bnconv2 = nn.BatchNorm2d(32, momentum=1.)
            
            self.conv3 = nn.Conv2d(32, 32, 3, stride=2, padding=1)
            self.bnconv3 = nn.BatchNorm2d(32, momentum=1.)
            
            self.conv4 = nn.Conv2d(32, 32, 3, stride=2, padding=1)
            self.bnconv4 = nn.BatchNorm2d(32, momentum=1.)
        ...
        self.bnfc1 = nn.BatchNorm1d(256, momentum=1.)
        
        self.fc2 = nn.Linear(256, num_outputs)
        self.bnfc2 = nn.BatchNorm1d(num_outputs, momentum=1.)
    ...
    def forward(self, inputs):
        if self.a3c_net:
            x = self.nonlin(self.bnconv1(self.conv1(inputs)))
            x = self.nonlin(self.bnconv2(self.conv2(x)))
        else:
            x = self.nonlin(self.bnconv1(self.conv1(inputs)))
            x = self.nonlin(self.bnconv2(self.conv2(x)))
            x = self.nonlin(self.bnconv3(self.conv3(x)))
            x = self.nonlin(self.bnconv4(self.conv4(x)))
        x = x.view(-1, self.num_conv_outputs)
        
        x = self.nonlin(self.bnfc1(self.fc1(x)))
